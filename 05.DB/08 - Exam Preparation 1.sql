...
-- 5 - Volunteers
SELECT [Name], PhoneNumber, [Address], AnimalId, DepartmentId FROM Volunteers ORDER BY [Name], AnimalId, DepartmentId
...
GROUP BY o.[Name] ORDER BY CountOfAnimals DESC, [Owner]
...
SELECT v.[Name], v.PhoneNumber, TRIM(REPLACE(REPLACE(v.[Address], 'Sofia', ' '), ',', ' ')) AS [Address] FROM Volunteers AS v
...
CREATE FUNCTION udf_GetVolunteersCountFromADepartment (@VolunteersDepartment VARCHAR(30)) RETURNS INT AS
BEGIN
...
(
SELECT COUNT(*) FROM Volunteers WHERE DepartmentId = @departmentId
)
